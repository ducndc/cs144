...
unsigned int g_ICMP_query_timeout_interval = 60;
unsigned int g_TCP_established_idle_timeout = 7440;
unsigned int g_TCP_transitory_idle_timeout = 300;

unsigned int g_ICMP_nat_port = 0;
unsigned int g_TCP_nat_port = 1024;
...
int main(int argc, char **argv)
{
    ...
    while ((c = getopt(argc, argv, "hs:v:p:u:t:r:l:I:R:E:T:n")) != EOF)
    {
        switch (c)
        {
            case 'h':
...
#if 1
            case 'I':
                g_ICMP_query_timeout_interval = atoi((char *)optarg);
                break;
            case 'E':
                g_TCP_established_idle_timeout = atoi((char *)optarg);
                break;
            case 'R':
                g_TCP_transitory_idle_timeout = atoi((char *)optarg);
                break;
#endif
        } /* switch */
    } /* -- while -- */
    ...
    if (g_is_nat)
    {
      printf("is_nat %d", g_is_nat);
      nat = calloc(1, sizeof(struct sr_nat));
      sr_nat_init(nat);
    }
