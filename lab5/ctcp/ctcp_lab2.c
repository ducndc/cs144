...
#define WRAPPED_CTCP_SEG_SIZE sizeof(wrapped_ctcp_segment_t)
 ...
 */
ctcp_state_t *state_list;

static void ctcp_send(ctcp_state_t *state);
static void ctcp_send_segment(ctcp_state_t *state,
...
static void ctcp_clean_up_unacked_segment_list(ctcp_state_t *state);
...
ctcp_state_t *ctcp_init(conn_t *conn, ctcp_config_t *cfg) {
  ...
  state->tx_state.unacked_segments = ll_create();
...
void ctcp_destroy(ctcp_state_t *state) {
  ...
  *state->prev = state->next;
  ...
  len = ll_length(state->tx_state.unacked_segments);

  for (i = 0; i < len; ++i)
  {
    ll_node_t *front_node_ptr = ll_front(state->tx_state.unacked_segments);
    free(front_node_ptr->object);
    ll_remove(state->tx_state.unacked_segments, front_node_ptr);
  }

  ll_destroy(state->tx_state.unacked_segments);
...
void ctcp_read(ctcp_state_t *state) {
  ...
  {
    new_segment_ptr =
      (wrapped_ctcp_segment_t*)calloc(1, WRAPPED_CTCP_SEG_SIZE + bytes_read);
    new_segment_ptr->ctcp_segment.len = htons((uint16_t)CTCP_SEGMENT_SIZE +
                                              bytes_read);
    new_segment_ptr->ctcp_segment.seqno = htonl(state->tx_state.last_seqno_read
                                                + 1);
    memcpy(new_segment_ptr->ctcp_segment.data, buf, bytes_read);
    state->tx_state.last_seqno_read += bytes_read;
    ll_add(state->tx_state.unacked_segments, new_segment_ptr);
  }

  if (bytes_read == -1)
  {
    state->tx_state.is_EOF = true;
    new_segment_ptr = (wrapped_ctcp_segment_t*)calloc(1, WRAPPED_CTCP_SEG_SIZE);
    new_segment_ptr->ctcp_segment.len = htons((uint16_t)CTCP_SEGMENT_SIZE);
    new_segment_ptr->ctcp_segment.seqno = htonl(state->tx_state.last_seqno_read
                                                + 1);
...
ctcp_send(ctcp_state_t *state)
{
  ...
  length = ll_length(state->tx_state.unacked_segments);
  if (length == 0)
  {
    return;
  }
  ...
  for (i = 0; i < length; ++i) {
    if (i == 0) {
      curr_node_ptr = ll_front(state->tx_state.unacked_segments);
    } else {
    ...
    else if (i == 0) 
    {
      ...
      ms_since_last_send = current_time() -
                           wrapped_ctcp_segment_ptr->timestamp_of_last_send;
...
ctcp_clean_up_unacked_segment_list(ctcp_state_t *state)
{
  ...
  while (ll_length(state->tx_state.unacked_segments) != 0) 
  {
    front_node_ptr = ll_front(state->tx_state.unacked_segments);
    wrapped_ctcp_segment_ptr = (wrapped_ctcp_segment_t*)front_node_ptr->object;
    num_data_bytes = ntohs(wrapped_ctcp_segment_ptr->ctcp_segment.len) -
                     CTCP_SEGMENT_SIZE;
...
ctcp_send_segment(ctcp_state_t *state, wrapped_ctcp_segment_t* wrapped_segment)
{
  ...
  if (wrapped_segment->num_xmits >= 10) 
  {
    ctcp_destroy(state);
  }

  wrapped_segment->ctcp_segment.ackno =
                               htonl(state->rx_state.last_seqno_accepted + 1);
  ...
  segment_cksum = cksum(&wrapped_segment->ctcp_segment,
...
void ctcp_receive(ctcp_state_t *state, ctcp_segment_t *segment, size_t len) {
  ...
  uint32_t last_seqno_of_segment;
  uint32_t largest_allowable_seqno;
  uint32_t smallest_allowable_seqno;
  ...
  num_data_bytes = ntohs(segment->len) - CTCP_SEGMENT_SIZE;
        ...
        for (i = 0; i < (length-1); ++i) 
        {
          ...
          else
          {
            if ((ntohl(segment->seqno) > ntohl(curr_ctcp_segment_ptr->seqno)) &&
                (ntohl(segment->seqno) < ntohl(next_ctcp_segment_ptr->seqno)))
            {
              ll_add_after(state->rx_state.segments_to_output, curr_node_ptr,
                           segment);
              break;
            }
          }
...
void ctcp_output(ctcp_state_t *state) {
  ...
  while (ll_length(state->rx_state.segments_to_output) != 0) 
  {
    ...
    num_data_bytes = ntohs(ctcp_segment_ptr->len) - CTCP_SEGMENT_SIZE;
    if (num_data_bytes) 
    {
      if (ntohl(ctcp_segment_ptr->seqno) != 
      ...
      return_value = conn_output(state->conn, ctcp_segment_ptr->data,
                                 num_data_bytes);
      ...
      if (return_value != num_data_bytes)
      {
        return;
      }
      num_segments_output++;
    ...
    if ((!state->rx_state.is_FIN) && (ctcp_segment_ptr->flags & TH_FIN))
    {
      state->rx_state.is_FIN = true;
...
ctcp_send_control_segment(ctcp_state_t *state)
{
  ...
  ctcp_segment.len   = CTCP_SEGMENT_SIZE;
  ctcp_segment.flags = TH_ACK;
  ctcp_segment.window = htons(state->ctcp_config.recv_window);
  ctcp_segment.cksum = 0;
  ctcp_segment.cksum = cksum(&ctcp_segment, CTCP_SEGMENT_SIZE);

  conn_send(state->conn, &ctcp_segment, CTCP_SEGMENT_SIZE);
}

static uint16_t 
ctcp_get_num_data_bytes(ctcp_segment_t* ctcp_segment_ptr)
{
  return ntohs(ctcp_segment_ptr->len) - CTCP_SEGMENT_SIZE;
}
...
void ctcp_timer() {
  ...
  for (curr_state = state_list; curr_state != NULL;
       curr_state = curr_state->next) 
  {
    ctcp_output(curr_state);
    ctcp_send(curr_state);

    if ((curr_state->rx_state.is_FIN)
      && (curr_state->tx_state.is_EOF)
      && (ll_length(curr_state->tx_state.unacked_segments) == 0)
      && (ll_length(curr_state->rx_state.segments_to_output) == 0)) 
    {
      ...
      else if ((current_time() - curr_state->FIN_WAIT_start_time) >
               (2 * MAX_SEG_LIFETIME_MS)) 
      {
